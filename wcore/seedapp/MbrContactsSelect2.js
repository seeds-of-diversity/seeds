/* Selector tool for choosing a contact from MbrContacts
   ...
   Usage:
       ...
       <script>let o = new MbrSelect2( { jSelect: $('#mbrChooseOne'),
                                         qUrl: '{$this->oP->oApp->UrlQ()}' } );
       </script>
*/

class MbrContactsSelect2
{
        ...
        this.raConfig.jSelect.select2({ 
              ajax:{ url: this.raConfig.qUrl,
                     ...
                     processResults: function (data) {
                                         ...
                                         let raResult = [];
                                         if( data.bOk ) {
                                             data.raOut.forEach( function(v, k, ra) { raResult.push({id:v._key, text:v.fullname+' ('+v._key+')'}); } );
                                         }
