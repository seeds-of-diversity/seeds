...
class MbrIntegrity
{
    ...
    function AssessDonations()
    {
        ...
        $sql = "SELECT M._key as kMbr, M.donation_date as date, M.donation as amount, M.donation_receipt as receipt_num
                FROM seeds_2.mbr_contacts M
                     LEFT JOIN seeds_2.mbr_donations D
        ...
        $sql = "SELECT B.uid_buyer as kMbr, DATE(O._created) as date, BP.f as amount, P.name as P_name
                FROM seeds_1.mbr_order_pending O
                     JOIN seeds_1.SEEDBasket_Baskets B ON (O.kBasket=B._key)
                     JOIN seeds_1.SEEDBasket_BP BP ON (B._key=BP.fk_SEEDBasket_Baskets)
                     JOIN seeds_1.SEEDBasket_Products P ON (P._key=BP.fk_SEEDBasket_Products)
                     LEFT JOIN seeds_2.mbr_donations D ON (D.fk_mbr_contacts=B.uid_buyer AND D.date_received=DATE(B._created) )
