...
class RosettaCultivarSynonyms
{
     ...
     */
    {
        ...
        $raNamesSrccv = $this->oApp->kfdb->QueryRowsRA(
                            "SELECT ocv,GROUP_CONCAT(_key,',') as k FROM seeds.sl_cv_sources
        ...
        if( ($jx = SEEDInput_Str('jx')) ) {
            ...
            switch( $jx ) {
                case 'makeSynonym':
                    ...
                    $raCV = $this->oApp->kfdb->QueryRA( "SELECT * from seeds.sl_pcv WHERE _key='$kPcv'" );
                    $raSrccv = $this->oApp->kfdb->QueryRA( "SELECT * from seeds.sl_cv_sources WHERE _key='$kSrccv'" );
                    $sSynName = addslashes($raSrccv['ocv']);
                    $this->oApp->kfdb->Execute( "INSERT INTO seeds.sl_pcv_syn (fk_sl_pcv,name,notes) VALUES ($kPcv,'$sSynName','')" );
